...
function loadDatabase(): TallerDatabase {
  if (!fs.existsSync(DB_PATH)) {
    const initialDb: TallerDatabase = {
      clientes: [
        {
          id: 'cli-1',
          cedula: 'V-19827364',
      ...
      reparaciones: [
        ...
        },
        {
          ...
          fecha_ingreso: '2026-04-10T10:00:00Z', // Ingreso antiguo para simular política de abandono
          ...
          trabajo_realizado: '',
          repuestos_utilizados: [],
          costo_mano_obra: 10.00, // Cargo mínimo por diagnóstico
          costo_repuestos: 0,
          costo_total: 10.00,
          descuento: 0,
          monto_final: 10.00,
          estado: 'reparado', // Listo para retiro pero sin respuesta
          prioridad: 'baja',
          tecnico_asignado: 'Daniel Romero (Master)',
          dias_garantia: 30,
          presupuesto_aceptado: true,
          fecha_aceptacion: '2026-04-11T11:00:00Z',
          fecha_abandono: '2026-05-11T11:00:00Z', // 30 días después
          estado_abandono: 'abandonado', // Reportará como chatarra o para despiece
          activo: true
        }
      ],
      ...
      facturas: [
        {
          ...
          subtotal: 120.69,
          iva: 19.31, // 16% IVA estándar Venezuela u otros locales
          total: 140.00,
...
function saveDatabase(db: TallerDatabase) {
...
app.post('/api/reparaciones', (req, res) => {
  ...
  const sameYearRepairs = db.reparaciones.filter(r => r.numero_orden.startsWith(`REP-${year}`));
  const nextNum = sameYearRepairs.length + 1;
  const formattedNum = String(nextNum).padStart(4, '0');
  const numero_orden = `REP-${year}-${formattedNum}`;
...
app.put('/api/reparaciones/:id', (req, res) => {
  ...
  if (estado === 'entregado' && prevDoc.estado !== 'entregado') {
    ...
    if (!existingFactura) {
      const facNum = `FAC-${year()}-${String(db.facturas.length + 1).padStart(4, '0')}`;
...
// HISTORIAL ENDPOINTS
app.get('/api/reparaciones/:id/historial', (req, res) => {
  const db = loadDatabase();
...
app.post('/api/reparaciones/:id/historial', (req, res) => {
  const db = loadDatabase();
